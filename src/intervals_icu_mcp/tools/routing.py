...
def cycling_route_to_gpx(
    ...
    *,
    ...
    training_block_sequence: TrainingBlockSequenceAnalysis | None = None,
) -> bytes:
    ...
    if training_window is not None and training_block_sequence is not None:
        raise ValueError(
            "GPX export accepts either a training window or a block sequence"
        )
    ...
    marker_indices: tuple[tuple[int, str], ...] = ()
    if training_window is not None:
        marker_indices = (
            (training_window.start.geometry_index, "TRAINING START"),
            (training_window.end.geometry_index, "TRAINING END"),
        )
    elif training_block_sequence is not None:
        marker_indices = tuple(
            marker
            for block_number, analysis in enumerate(
                training_block_sequence.work_blocks,
                start=1,
            )
            for marker in (
                (
                    analysis.window.start.geometry_index,
                    f"BLOCK {block_number} START",
                ),
                (
                    analysis.window.end.geometry_index,
                    f"BLOCK {block_number} END",
                ),
            )
        )
    ...
    for geometry_index, marker_name in marker_indices:
        if not 0 <= geometry_index < len(route.geometry):
            raise ValueError("Training geometry index is outside the route")
    ...
    try:
        ...
        if include_gpx:
            ...
            gpx_content = cycling_route_to_gpx(
                best_analysis.candidate.route,
                name=f"Cycling training route {best_analysis.candidate.candidate_id}",
                description=(
                    "Generated by intervals-icu-mcp with training markers."
                ),
                training_window=(
                    best_analysis.best_training_window.window
                    if block_sequence is None
                    else None
                ),
                training_block_sequence=block_sequence,
            )
            serialized[0]["gpx"] = {
                "format": "GPX 1.1",
                "encoding": "base64",
